...
class Hyperparameters:
    ...
    mtp_loss_weight_2: float = float(os.environ.get("MTP_LOSS_WEIGHT_2", 0.03))
    mtp_disable_on_warmdown: bool = bool(int(os.environ.get("MTP_DISABLE_ON_WARMDOWN", "1")))
    ...
    def mtp_loss_weight_2_for(self, base_lr_mul: float) -> float:
        weight = max(self.mtp_loss_weight_2, 0.0)
        if weight <= 0.0:
            return 0.0
        if self.mtp_disable_on_warmdown and base_lr_mul < 1.0:
            return 0.0
        return weight
...
class GPT(nn.Module):
    ...
    def _loss_sum_from_hidden(self, hidden: mx.array, target_ids: mx.array) -> tuple[mx.array, float]:
        ...
        if self.logit_chunk_tokens <= 0 or x.shape[0] <= self.logit_chunk_tokens:
            ...
            return nn.losses.cross_entropy(logits.astype(mx.float32), y, reduction="sum"), token_count
    ...
    def _loss_sum_from_logits(self, logits: mx.array, target_ids: mx.array) -> tuple[mx.array, float]:
        y = target_ids.reshape(-1)
        flat_logits = logits.reshape(-1, logits.shape[-1])
        return nn.losses.cross_entropy(flat_logits.astype(mx.float32), y, reduction="sum"), float(y.size)

    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        # Eval/export stay on pure next-token loss so score comparisons remain apples-to-apples.
        hidden = self(input_ids)
        loss_sum, token_count = self._loss_sum_from_hidden(hidden, target_ids)
        return loss_sum / token_count

    def training_loss(self, input_ids: mx.array, target_ids: mx.array, mtp_loss_weight_2: float = 0.0) -> mx.array:
        # Keep the train-only auxiliary weight out of model.state so export stays unchanged.
        hidden = self(input_ids)
        total_loss, total_weight = self._loss_sum_from_hidden(hidden, target_ids)
        if mtp_loss_weight_2 <= 0.0 or hidden.shape[1] <= 1:
            return total_loss / total_weight

        batch, seqlen, dim = hidden.shape
        flat_hidden = hidden.reshape(-1, dim)
        if self.logit_chunk_tokens <= 0 or flat_hidden.shape[0] <= self.logit_chunk_tokens:
            logits_proj = flat_hidden @ self.tok_emb.weight.astype(flat_hidden.dtype).T
            logits = self.softcap(logits_proj).reshape(batch, seqlen, -1)
            aux_loss, aux_weight = self._loss_sum_from_logits(logits[:, :-1, :], target_ids[:, 1:])
        else:
            aux_loss, aux_weight = self._loss_sum_from_hidden(hidden[:, :-1, :], target_ids[:, 1:])
        total_loss = total_loss + aux_loss * mtp_loss_weight_2
        total_weight += aux_weight * mtp_loss_weight_2
        return total_loss / total_weight
...
def main() -> None:
    ...
    mtp_loss_weight_2 = max(args.mtp_loss_weight_2, 0.0)
    ...
    compiled_mtp_loss_and_grad = None
    if mtp_loss_weight_2 > 0.0:
        compiled_mtp_loss_and_grad = mx.compile(
            nn.value_and_grad(model, lambda x, y: model.training_loss(x, y, mtp_loss_weight_2)),
            inputs=model.state,
            outputs=model.state,
        )
    ...
    log(
        f"train_objective:next_token+mtp_t2_pre_warmdown "
        f"mtp_loss_weight_2:{mtp_loss_weight_2} "
        f"mtp_disable_on_warmdown:{args.mtp_disable_on_warmdown}"
    )
    ...
    if args.warmup_steps > 0:
        ...
        warmup_train_loss_and_grad = compiled_mtp_loss_and_grad or compiled_loss_and_grad
        for warmup_step in range(args.warmup_steps):
            accum: dict[str, mx.array] | None = None
            warmup_loss = mx.array(0.0, dtype=mx.float32)
            grad_scale = 1.0 / args.grad_accum_steps
            for _ in range(args.grad_accum_steps):
                warmup_loss, grads = loss_and_grad_chunked(args, train_loader, warmup_train_loss_and_grad)
        ...
        if compiled_mtp_loss_and_grad is not None:
            warm_train_loss, warm_train_grads = loss_and_grad_chunked(args, train_loader, compiled_loss_and_grad)
            mx.eval(warm_train_loss, warm_train_grads)
            mx.synchronize()
    ...
    step = 0
    mtp_active_logged: bool | None = None
    while True:
        ...
        use_mtp = compiled_mtp_loss_and_grad is not None and args.mtp_loss_weight_2_for(base_lr_mul) > 0.0
        if use_mtp != mtp_active_logged:
            mode = "next_token+mtp_t2" if use_mtp else "next_token"
            log(f"train_objective_phase:{mode} step:{step} base_lr_mul:{base_lr_mul:.6f}")
            mtp_active_logged = use_mtp
        compiled_train_loss_and_grad = compiled_mtp_loss_and_grad if use_mtp else compiled_loss_and_grad
