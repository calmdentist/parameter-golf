...
class Hyperparameters:
    ...
    mtp_loss_weight_2: float = float(os.environ.get("MTP_LOSS_WEIGHT_2", 0.15))
    mtp_loss_weight_3: float = float(os.environ.get("MTP_LOSS_WEIGHT_3", 0.05))
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 ...
                 qk_gain_init: float, mtp_loss_weights: tuple[tuple[int, float], ...] = ()):
        super().__init__()
        if logit_softcap <= 0.0:
            raise ValueError(f"logit_softcap must be positive, got {logit_softcap}")
        self.logit_chunk_tokens = logit_chunk_tokens
        self.logit_softcap = logit_softcap
        self.mtp_loss_weights = tuple((offset, weight) for offset, weight in mtp_loss_weights if offset > 1 and weight > 0.0)
    ...
    def _loss_sum_from_logits(self, logits: mx.array, target_ids: mx.array) -> tuple[mx.array, float]:
        y = target_ids.reshape(-1)
        return nn.losses.cross_entropy(logits.astype(mx.float32), y, reduction="sum"), float(y.size)
    ...
    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        # Eval/export remain on the original next-token objective so score comparisons stay apples-to-apples.
    ...
    def training_loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        # Train-only multi-token prediction improves sample efficiency without changing export bytes.
        hidden = self(input_ids)
        if not self.mtp_loss_weights:
            loss_sum, token_count = self._loss_sum_from_hidden(hidden, target_ids)
            return loss_sum / token_count
        ...
        if self.logit_chunk_tokens <= 0 or flat_hidden.shape[0] <= self.logit_chunk_tokens:
            ...
            total_loss, total_weight = self._loss_sum_from_logits(logits.reshape(-1, logits.shape[-1]), target_ids)
            for offset, weight in self.mtp_loss_weights:
                shift = offset - 1
                if seqlen <= shift:
                    continue
                aux_loss, aux_weight = self._loss_sum_from_logits(
                    logits[:, :-shift, :].reshape(-1, logits.shape[-1]),
                    target_ids[:, shift:],
                )
                total_loss = total_loss + aux_loss * weight
                total_weight += aux_weight * weight
        ...
        for offset, weight in self.mtp_loss_weights:
            ...
            if seqlen <= shift:
                continue
            aux_loss, aux_weight = self._loss_sum_from_hidden(hidden[:, :-shift, :], target_ids[:, shift:])
            total_loss = total_loss + aux_loss * weight
            total_weight += aux_weight * weight
        return total_loss / total_weight
...
def main() -> None:
    ...
    model = GPT(
        ...
        mtp_loss_weights=((2, args.mtp_loss_weight_2), (3, args.mtp_loss_weight_3)),
    )
    ...
    log(
        f"train_objective:next_token+mtp "
        f"mtp_loss_weight_2:{args.mtp_loss_weight_2} "
        f"mtp_loss_weight_3:{args.mtp_loss_weight_3}"
    )
