...
class Hyperparameters:
    ...
    scalar_max_matrix_lr_ratio: float = float(os.environ.get("SCALAR_MAX_MATRIX_LR_RATIO", 4.0))
    ...
    def scalar_lr_mul(self, base_lr_mul: float, matrix_lr_mul: float | None = None) -> float:
        scalar_mul = self.floored_lr_mul(base_lr_mul, self.scalar_lr_floor_ratio)
        if (
            base_lr_mul >= 1.0
            or matrix_lr_mul is None
            or self.scalar_max_matrix_lr_ratio <= 0.0
            or self.scalar_lr <= 0.0
            or self.matrix_lr <= 0.0
        ):
            return scalar_mul
        max_scalar_mul = (
            self.scalar_max_matrix_lr_ratio * self.matrix_lr * matrix_lr_mul / self.scalar_lr
        )
        return min(scalar_mul, max_scalar_mul)
...
def main() -> None:
    ...
    log(
        ...
        f"scalar_max_matrix_lr_ratio:{args.scalar_max_matrix_lr_ratio} "
    ...
    while True:
        ...
        scalar_lr_mul = args.scalar_lr_mul(base_lr_mul, matrix_lr_mul=matrix_lr_mul)
