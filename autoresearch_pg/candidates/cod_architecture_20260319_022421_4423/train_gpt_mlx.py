...
class Hyperparameters:
    ...
    normalize_skip_connections: bool = bool(int(os.environ.get("NORMALIZE_SKIP_CONNECTIONS", "1")))
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 ...
                 qk_gain_init: float, normalize_skip_connections: bool):
        super().__init__()
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        ...
        for i in range(self.num_decoder_layers):
            ...
            if skips:
                skip = skips.pop()
                if self.normalize_skip_connections:
                    # Normalize reused encoder activations so decoder skip fusion stays on a more
                    # stable scale than the raw hidden-state magnitude from each encoder depth.
                    skip = rms_norm(skip)
...
def main() -> None:
    ...
    model = GPT(
        ...
        normalize_skip_connections=args.normalize_skip_connections,
    )
    ...
    log(
        ...
        f"normalize_skip_connections:{args.normalize_skip_connections}"
    )
    log(
