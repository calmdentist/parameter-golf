...
class Hyperparameters:
    ...
    int8_qat_start_frac: float = float(os.environ.get("INT8_QAT_START_FRAC", 0.88))
    int8_qat_interval: int = int(os.environ.get("INT8_QAT_INTERVAL", 4))
    int8_qat_min_numel: int = int(os.environ.get("INT8_QAT_MIN_NUMEL", 262_144))
    int8_qat_include_embedding: bool = bool(int(os.environ.get("INT8_QAT_INCLUDE_EMBEDDING", "0")))
    ...
    def int8_qat_should_project(self, next_step: int, elapsed_ms: float) -> bool:
        if self.int8_qat_interval <= 0 or next_step % self.int8_qat_interval != 0:
            return False
        # Under MLX wallclock-capped runs, lr_mul<1 can start much earlier than the
        # intended final 10-15%, so gate QAT by absolute run progress instead.
        start_frac = min(max(self.int8_qat_start_frac, 0.0), 1.0)
        if self.max_wallclock_seconds > 0:
            return elapsed_ms >= start_frac * 1000.0 * self.max_wallclock_seconds
        start_step = max(1, math.ceil(start_frac * max(self.iterations, 1)))
...
def build_int8_qat_keys(model: GPT, opt: SplitOptimizers, args: Hyperparameters) -> list[str]:
    params = dict(tree_flatten(model.parameters()))
    candidate_keys = list(opt.matrix_keys)
    if args.int8_qat_include_embedding:
        candidate_keys.insert(0, opt.embed_key)
    return [
        name
        for name in candidate_keys
        if params[name].ndim == 2 and int(params[name].size) >= args.int8_qat_min_numel
    ]


def project_int8_qat_tensors(model: GPT, qat_keys: list[str]) -> int:
    if not qat_keys:
        return 0
    params = dict(tree_flatten(model.parameters()))
    projected: dict[str, mx.array] = {}
    for name in qat_keys:
        arr = params[name]
        q, s = quantize_float_array(arr)
        projected[name] = dequantize_float_array(q, s, str(arr.dtype).split(".")[-1])
    if projected:
...
def dequantize_state_dict_int8(quant_obj: dict[str, object]) -> dict[str, mx.array]:
    ...
    for name, q in quant_obj["quantized"].items():
        ...
        out[name] = dequantize_float_array(np.asarray(q, dtype=np.int8), quant_obj["scales"][name], dtype_name)
...
def main() -> None:
    ...
    int8_qat_keys = build_int8_qat_keys(model, opt, args)
    ...
    log(
        f"int8_qat:start_frac:{args.int8_qat_start_frac:.3f} interval:{args.int8_qat_interval} "
        f"min_numel:{args.int8_qat_min_numel} include_embedding:{args.int8_qat_include_embedding} "
        f"projected_tensor_candidates:{len(int8_qat_keys)}"
    )
    ...
    while True:
        ...
        projected_now = 0
        if args.int8_qat_should_project(step + 1, elapsed_ms):
            projected_now = project_int8_qat_tensors(model, int8_qat_keys)
            if projected_now:
                int8_qat_projection_count += projected_now
                if int8_qat_projection_count == projected_now:
                    log(
                        f"int8_qat:active start_frac:{args.int8_qat_start_frac:.3f} "
                        f"interval:{args.int8_qat_interval} first_step:{step + 1} "
                        f"projected_tensors:{projected_now}"
                    )
        mx.synchronize()
    ...
    # ==============================================================================
    log(f"int8_qat_projected_tensors_total:{int8_qat_projection_count}")
